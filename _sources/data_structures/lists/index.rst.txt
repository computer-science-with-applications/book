...
Earlier in this chapter, we mentioned that it is possible to introduce
aliases--a second name for a data structure--using an assignment
...
``lst1`` and ``lst2`` are aliases: they both refer to the same list,
which contains the values ``1``, ``2``, etc.

Function parameters provide another way to introduce aliases.

For
example, here's a function, ``remove_vals``, that takes two lists and
removes that values from ``lst2`` that appear in ``lst1``:

.. python-run::
   :formatting: separate
		
   def remove_vals(lst1, lst2):
       """
       Remove the values that appear in lst2 from lst1.

       Inputs:
         lst1: a list of unique values
         lst2: a list of unique values
...
Notice that the docstring states that the inputs to this function are
expected to be lists of unique values, that is, any given value
appears at most once in a list.  This restriction is included because
the ``remove`` method removes the first occurrence of a value in a
list, not all of the occurrences of a value in a list.  If we wanted
to allow a value to appear (and be removed) multiple times, we'd need
to add an inner loop.

Let's first look at a call to this function that uses two different lists:
...
.. python-run::
   ...
   lstA = [1, 2, 3, 4]
   lstB = [2, 4]
   remove_vals(lstA, lstB)


During the call to ``remove_vals``, memory will look like this:


After the call, it will look like this:

Notice that ``lstA`` has fewer elements: the two values that appeared
in ``lstB`` have been removed as expected.

Now let's look at a call to this function that uses the *same* list for both arguments:

.. python-run::
   :formatting: separate

   lst3 = [10, 20, 30, 40]
   remove_vals(lst3, lst3)
   print("lst3 after the call to remove_vals(lst3, lst3):", lst3)

It is natural to expect the result of the function to be the empty
list (``[]``), since the all the elements in the first parameter occur
in the second parameter.  But since ``lst1`` and ``lst2`` inside the
function are aliases for the same list, the result will be
unpredictable.  We 
